...
class _Client {
  ...
  static Future<dynamic> get(String path) async {
    if (MockData.isDemoMode) {
      await Future.delayed(const Duration(milliseconds: 200));
      return jsonDecode(jsonEncode(MockData.handleGet(path)));
    }
    try {
  ...
  static Future<dynamic> post(String path, Map<String, dynamic> body) async {
    if (MockData.isDemoMode) {
      await Future.delayed(const Duration(milliseconds: 200));
      return jsonDecode(jsonEncode(MockData.handlePost(path, body)));
    }
    try {
  ...
  static Future<dynamic> patch(String path, Map<String, dynamic> body) async {
    if (MockData.isDemoMode) {
      await Future.delayed(const Duration(milliseconds: 200));
      return jsonDecode(jsonEncode(MockData.handlePost(path, body))); // reuse post handler for patch
    }
    try {
...
class ApiService {
  ...
  String get baseUrl => 'http://${_Config.gatewayHost}:4000';
  ...
  static Future<String> testGatewayConnection() async {
    if (MockData.isDemoMode) return 'Demo mode active (Offline).';
    try {
...
class OcrApi {
  static String get _ocrBaseUrl => 'http://${_Config.gatewayHost}:8001';

  Future<List<dynamic>> extractMedicines(String imagePath) async {
    if (MockData.isDemoMode) {
      await Future.delayed(const Duration(seconds: 1));
      return ['Panadol Advance', 'Augmentin 1g', 'Concor 5mg'];
    }
...
class _MedicinesApi {
  Future<MedicineListResponse> getAll({
    ...
    final json = await _Client.get('/api/medicines?$queryString') as Map<String, dynamic>;
...
class _InvoicesApi {
  ...
  Future<Map<String, dynamic>> importExcel(List<int> fileBytes, String filename, {String? companyName, String? fatooraNumber}) async {
    if (MockData.isDemoMode) {
      await Future.delayed(const Duration(seconds: 1));
      return {'success': true, 'message': 'Demo mock import success'};
    }
    final uri = Uri.parse(
  ...
  Future<Map<String, dynamic>> uploadInvoiceImage(String imagePath) async {
    if (MockData.isDemoMode) {
      await Future.delayed(const Duration(seconds: 1));
      return {'success': true, 'message': 'Demo mock upload success'};
    }
    final uri = Uri.parse(
  ...
  void connect() {
    if (MockData.isDemoMode) {
      ...
      _demoTimer = Timer.periodic(const Duration(seconds: 4), (timer) {
        for (var job in MockData.activeJobs) {
          if (job['status'] == 'PENDING') {
            job['status'] = 'DISPATCHED';
            if (job['pick_sequence'] != null) {
              for (var item in job['pick_sequence']) item['status'] = 'SENDING';
            }
            onEvent({'type': 'ROBOT_ACK', 'jobId': job['job_id'], 'status': 'DISPATCHED', 'message': 'Arm moving to fetch items'});
          } else if (job['status'] == 'DISPATCHED') {
            job['status'] = 'COMPLETED';
            if (job['pick_sequence'] != null) {
              for (var item in job['pick_sequence']) item['status'] = 'DONE';
            }
            onEvent({'type': 'ROBOT_ACK', 'jobId': job['job_id'], 'status': 'COMPLETED', 'message': 'Dispensing complete'});
          }
        }
      });
      return;
