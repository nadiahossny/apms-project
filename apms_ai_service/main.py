...
answer_prompt = PromptTemplate.from_template(
    """Given the following user question, corresponding SQL query, and SQL result, answer the user question in a friendly, helpful, and concise manner tailored for a pharmacist or pharmacy manager.
Focus on the business insights from the data.
Keep your answer VERY short (2-3 sentences max). Use Markdown format: use bullet points (-) for lists and **bold text** to highlight important numbers, warnings, or things that need immediate attention.
DO NOT use technical jargon, do not mention SQL, and do not explain how the data was fetched. Speak naturally as a helpful pharmacy assistant.
